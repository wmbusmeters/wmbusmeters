...
bool DVEntry::extractDate(struct tm *out)
{
    ...
    if (v.size() == 2) {
        ...
        int sec  = (0x3f) & v[0];
        out->tm_sec = sec;
        // EN 13757-3 Type I (CP48): byte[1] bit 7 is the time-valid flag.
        // Matching the libmbus reference implementation (mbus_data_tm_decode),
        // an invalid timestamp is not emitted.
        if (v[1] & 0x80) ok = false;
        // The remaining CP48 metadata bits (daylight saving at byte[0] bit 6,
        // day-of-week, leap-year, week-of-year) are deliberately not written
        // here. wmbusmeters renders this struct tm through an mktime ->
        // localtime_r round trip, so setting tm_isdst would shift the
        // displayed hour (e.g. 19:59 -> 20:59) depending on the local time
        // zone. Surfacing these flags correctly would require a separate
        // metadata field that does not go through tm_isdst. TODO.
    }

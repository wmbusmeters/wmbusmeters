...
void MeterQHeat::processContent(Telegram *t)
{
    ...
    */
    ...
    if(findKey(MeasurementType::Instantaneous, VIFRange::EnergyWh, 1, 0, &key, &t->dv_entries)) {
        extractDVdouble(&t->dv_entries, key, &offset, &last_year_energy_kwh_);
    ...
    if(findKey(MeasurementType::Instantaneous, VIFRange::EnergyWh, 17, 0, &key, &t->dv_entries)) {
        extractDVdouble(&t->dv_entries, key, &offset, &last_month_energy_kwh_);

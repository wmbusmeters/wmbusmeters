...
then
    printOK "too short izar telegram"
else
    cat $TEST/test_output.txt
    printERROR "failed to handle too short izar telegram."
    exit 1
fi
...
then
    printOK "too short qheatv2 telegram forced to be analyzed with qheat."
else
    cat $TEST/test_output.txt
    printERROR "failed to handle too short qheat telegram during analyze."
    exit 1
fi

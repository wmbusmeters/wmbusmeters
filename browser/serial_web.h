...
struct SerialCommunicationManagerImp : public SerialCommunicationManager
{
    ...
    // Re-entrancy guard: timer callbacks must not fire while a timer callback
    // is already executing (that would cause recursive device detection, which
    // overflows the ASYNCIFY stack and produces "function signature mismatch").
    int timer_depth_ = 0;
    ...
    // Called from waitForStop — does everything:
    // drain disconnects, fire timers, check pending data
    void tickCallbacks();

    // Called from Semaphore::wait — only drains data, does NOT fire timers.
    // Avoids recursive device-detection while a probe is already in progress.
    void tickDataOnly();
